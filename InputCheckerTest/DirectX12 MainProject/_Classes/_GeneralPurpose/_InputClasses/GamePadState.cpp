...
void GamePadState::Accepts(int index) {
	...
	*/
	...
	flag_ += DXTK->GamePadState[index].dpad.up ? (1 << UP) : 0;
	flag_ += DXTK->GamePadState[index].dpad.down ? (1 << DOWN) : 0;
	flag_ += DXTK->GamePadState[index].dpad.left ? (1 << LEFT) : 0;
	flag_ += DXTK->GamePadState[index].dpad.right ? (1 << RIGHT) : 0;
